...
- review any relevant architecture docs and follow their constraints
- link the relevant architecture docs from the PRD
- if the main work is defining a new system design, boundary, or runtime contract, use `arch-create` to create or update the architecture doc first, then write the PRD against that design

## Process

### Step 1: Understand the Feature Concept
Ask the user to describe the feature idea to understand the core concept and scope.
Determine whether the main artifact should be a PRD, an architecture doc, or both.
If the feature depends on a new system design or major boundary change, use `arch-create` to create or update the architecture doc first and then draft the PRD against it.
...
3. **Architecture Context**: "Which existing architecture docs govern this work, and do we need a new or updated architecture doc first?"
4. **Technical Scope**: "What are the core technical changes required?"
5. **Documentation Impact**: "Which existing docs need updates? What new docs are needed?"
6. **Integration Points**: "How does this feature integrate with existing systems?"
7. **Success Criteria**: "How will we know this feature is working well?"
8. **Implementation Phases**: "How can we deliver value incrementally?"
9. **Risk Assessment**: "What are the main risks and how do we mitigate them?"
10. **Dependencies**: "What other systems or features does this depend on?"
11. **Validation Strategy**: "How will we test and validate the implementation?"
...
2. **Architecture Check**: Review relevant architecture docs and decide whether `arch-create` should be used to create or update an architecture doc before the PRD
3. **Create GitHub Issue FIRST**: Short, stable concept description to get issue ID
4. **Create PRD File**: Detailed document using actual issue ID: `prds/[issue-id]-[feature-name].md`
5. **Link Architecture Docs**: Reference the relevant architecture docs from the PRD
6. **Update GitHub Issue**: Add link to PRD file now that filename is known
7. **Section-by-Section Discussion**: Work through each template section systematically
8. **Milestone Definition**: Define 5-10 major milestones that represent meaningful progress
9. **Review & Validation**: Ensure completeness and clarity
