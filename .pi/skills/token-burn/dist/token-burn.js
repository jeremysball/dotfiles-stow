 ...
 */
async function* streamJsonlLines(filepath) {
 ...
 */
function getModelName(provider, modelId) {
    if (provider && modelId) {
        if (modelId.includes('/'))
            return modelId;
    ...
    for await (const line of streamJsonlLines(filepath)) {
        ...
        try {
            ...
            if (msgType === 'custom' && data.customType === 'model-snapshot') {
                ...
                if (model)
                    currentModel = getModelName(provider, model);
            }
 ...
 */
function findSessionFiles(basePath) {
    const files = [];
...
//# sourceMappingURL=token-burn.js.map