...
> Example PRD numbers in this skill are placeholders. Do not anchor on them. Always prefer the PRD most clearly established by the current conversation, branch, or modified files.

1. **Git Branch Analysis** - Check current branch name for PRD patterns:
   - `feature/prd-<id>-*` → PRD <id>
   - `prd-<id>-*` → PRD <id>
   - `feature/prd-*` → Extract PRD number

2. **Recent Git Commits** - Look at recent commit messages for PRD references:
   - "fix: PRD <id> documentation" → PRD <id>
   - "feat: implement prd-<id> features" → PRD <id>

3. **Git Status Analysis** - Check modified/staged files for PRD clues:
   - Modified `prds/<id>-*.md` → PRD <id>
...
**Detection Logic:**
- **High Confidence**: Branch name matches PRD pattern (e.g., `feature/prd-<id>-feature-name`)
...
```markdown
🎯 **Auto-detected PRD <id>** (Feature Name)
- Branch: `feature/prd-<id>-feature-name` ✅
- Modified files: `prds/<id>-feature-name.md` ✅
- Recent commits mention PRD <id> features ✅
```
